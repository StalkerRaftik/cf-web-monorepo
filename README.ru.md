...
Современное одностраничное веб-приложение на базе Cloudflare Workers с единым репозиторием.
...
2. **Подготовка папки dist:**
   - Бэкенду требуется наличие папки `dist` перед запуском (он загружает ассеты оттуда)
